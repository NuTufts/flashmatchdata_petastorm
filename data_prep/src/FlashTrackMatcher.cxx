...
FlashTrackMatcher::FlashTrackMatcher(const FlashMatchConfig& config)
    : config_(config), total_tracks_(0), matched_tracks_(0), 
      total_flashes_(0), matched_flashes_(0), crt_matched_tracks_(0) {
}

std::vector<FlashTrackMatch> FlashTrackMatcher::FindMatches(const EventData& event_data) {
    ...
    for (size_t track_idx = 0; track_idx < event_data.cosmic_tracks.size(); ++track_idx) {
        const auto& track = event_data.cosmic_tracks[track_idx];
...
FlashTrackMatch FlashTrackMatcher::MatchTrackToFlash(const CosmicTrack& track,
    ...
    for (size_t flash_idx = 0; flash_idx < flashes.size(); ++flash_idx) {
        const auto& flash = flashes[flash_idx];
        ...
        double time_diff = std::min(time_diff_anode, time_diff_cathode);
...
double FlashTrackMatcher::CalculateAnodeCrossingTime(const CosmicTrack& track) const {
...
double FlashTrackMatcher::CalculateCathodeCrossingTime(const CosmicTrack& track) const {
    // Calculate time when track crosses cathode (X = CATHODE_X)
    
    double cathode_distance = std::abs(track.start_point.X() - CATHODE_X);
    if (track.direction.X() != 0) {
        return cathode_distance / (config_.drift_velocity * std::abs(track.direction.X()));
    }
    
    return track.cathode_crossing_time; // Use existing value if available
}

double FlashTrackMatcher::CalculateSpatialDistance(const CosmicTrack& track, 
...
double FlashTrackMatcher::CalculateMatchScore(const CosmicTrack& track,
...
int FlashTrackMatcher::FindCRTTrackMatch(const CosmicTrack& cosmic_track,
...
std::vector<int> FlashTrackMatcher::FindCRTHitMatches(const CosmicTrack& cosmic_track,
...
std::vector<FlashTrackMatch> FlashTrackMatcher::ResolveDegeneracies(
    ...
    for (const auto& flash_group : flash_to_matches) {
...
void FlashTrackMatcher::UpdateConfig(const FlashMatchConfig& config) {
    config_ = config;
}

bool FlashTrackMatcher::LoadConfigFromFile(const std::string& filename) {
...
std::map<std::string, double> FlashTrackMatcher::GetMatchingStatistics() const {
...
void FlashTrackMatcher::PrintStatistics() const {
...
bool FlashTrackMatcher::IsTimingCompatible(const CosmicTrack& track, 
                                          const OpticalFlash& flash) const {
    double time_diff_anode = std::abs(flash.flash_time - track.anode_crossing_time);
    double time_diff_cathode = std::abs(flash.flash_time - track.cathode_crossing_time);
...
bool FlashTrackMatcher::IsSpatiallyCompatible(const CosmicTrack& track,
                                             const OpticalFlash& flash) const {
    double spatial_dist = CalculateSpatialDistance(track, flash);
    return spatial_dist <= config_.track_flash_distance_cut;
}

double FlashTrackMatcher::CalculatePMTCoverage(const CosmicTrack& track,
                                              const OpticalFlash& flash) const {
...
std::vector<double> FlashTrackMatcher::ProjectTrackToPMTs(const CosmicTrack& track) const {
...
double FlashTrackMatcher::CalculateChiSquare(const std::vector<double>& predicted,
