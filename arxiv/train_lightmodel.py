...
print("TRAIN LIGHT-MODEL URESNET")
...
from flashmatchnet.losses.loss_poisson_emd_lm import PoissonNLLwithEMDLoss
...
checkpoint_folder = "/cluster/tufts/wongjiradlabnu/pabrat01/ubdl/flashmatchdata_petastorm/checkpoints/"
LOAD_FROM_CHECKPOINT=True
checkpoint_file=checkpoint_folder+"/lightmodel_mlp_iter_30000.pth"
start_iteration = 30001 #set to +1 after nth iteration
...
for iteration in range(start_iteration,end_iteration): 
    ...
    ibatchsize=start_per_batch.shape[0]
    ...
    featList.append( q_feat )
    print("featList: ", featList)
    print("len(featList): ", len(featList))
    '''

    #coords, feats = ME.utils.sparse_collate( coords=coordList, feats=featList, device=device )
    '''
    print("coords: ", coords)
    ...
    coord_un = torch.unique(coord, dim=0)
    print("coord unique shape: ", coord_un.shape)

    print("type(coord) ", type(coord) )

    input = ME.SparseTensor(features=q_feat, coordinates=coord)
    ...
    #forward
    pe_per_voxel = net(input)



    print("Printing output. Is it all between 0 and 1?")
    print("Output: ", pe_per_voxel)
    print("Output: ", pe_per_voxel.shape)
    ...
    '''
    print("ibatchsize: ", ibatchsize)

    outpe_allbatches = torch.zeros( (ibatchsize,32) )
    for ib in range(ibatchsize):

        print("this is ib:", ib)
        print("start_per_batch[ib]: ", start_per_batch[ib])
        print("end_per_batch[ib]: ", end_per_batch[ib])

        q_ib = q_feat[ start_per_batch[ib]:end_per_batch[ib],:]

        print("q_ib = q_feat[ start_per_batch[ib]:end_per_batch[ib],:]: ", q_ib)
        ...
        pe_ib = torch.transpose( pe_per_voxel[ start_per_batch[ib]:end_per_batch[ib],:], 1,0)

        print("Actual net output for this respective N: ", pe_per_voxel[ start_per_batch[ib]:end_per_batch[ib],:])
        print("Actual net output shape: ", pe_per_voxel.shape)
        print("Actual net output shape for this N: ", pe_per_voxel[ start_per_batch[ib]:end_per_batch[ib],:].shape)
        ...
        outpe_allbatches[ib,:] = torch.sum(output, 1)

        print("summed output along N (in main script): ", output)
        print("shape of summed output along N: ", output.shape)

    print("outpe_allbatches.shape: ", outpe_allbatches.shape)
    print("outpe_allbatches: ", outpe_allbatches)
    '''
    ...
    if iteration%CHECKPOINT_NITERS==0 and iteration>start_iteration:
        ...
        torch.save({'epoch': epoch,
                    ...
                    'loss': loss_tot}, 
	           checkpoint_folder+'/lightmodel_uresnet_iter_%d.pth'%(int(iteration)))
...
torch.save({'epoch': epoch,
            ...
            'optimizer_state_dict': optimizer.state_dict(),
            'loss': loss_tot}, 
	   checkpoint_folder+'/lightmodel_uresnet_enditer_%d.pth'%(int(end_iteration)))
