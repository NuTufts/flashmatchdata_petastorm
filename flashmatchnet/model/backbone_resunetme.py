...
class MinkEncode4Layer(MinkEncodeBase):
    BLOCK = BasicBlockInstanceNorm
    LAYERS = ( 1,  1,  1,   1)
    PLANES = (16, 32, 64, 128)
    INIT_DIM = 16

class MinkDecode4Layer(MinkDecodeBase):
    BLOCK = BasicBlockInstanceNorm
    IN_PLANES = (16, 32, 64, 128)
    LAYERS = (1, 1, 1, 1)  
    PLANES = (64, 32, 16, 16)
    INIT_DIM = 16

# class MEResUNet4Layer(nn.Module):
#     """
#     Residual UNet built using Minkowski engine
#     """
#     def __init__(self,in_channels=1, out_channels=16, D=3):
#         super(MinkEncodeDecodeUNet34,self).__init__()    
#         self.encoder = MinkEncode4Layer(in_channels=in_channels, out_channels=out_channels, D=D)
#         self.decoder = MinkDecode4Layer(in_channels=in_channels, out_channels=out_channels, D=D)
