...
class FlashMatchMLP(nn.Module):
    ...
    def __init__(self,input_nfeatures=100,
        ...
        if norm_layer=='batchnorm':
            ...
            raise ValueError(f"Unsupported normalization layer chosen: {norm_layer}")
        ...
        for ilayer,nfeat in enumerate(hidden_layer_nfeatures[1:]):
            ...
            seq.add_module( 'layer%d_norm'%(ilayer), norm(nfeat) )
    ...
    def forward(self,x, q):
        ...
        out = self.mlp(x)
        # honestly, below looks daft
        # the visibility function should be clamped, while the light yield scale ...
