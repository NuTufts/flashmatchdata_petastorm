...
if __name__ == "__main__":
    ...
    for ientry in range(nentries):
        ...
        ev_chstatus = io.get_data( "chstatus", "wire" )
        ...
        for p in range(img_v.size()):
            ...
            results = forwardpass( p, nrows, ncols, sparseimg_v, weights[p] )
        ...
        evout_input = outlcv.get_data( "image2d", "wire" )
        evout_chstatus = outlcv.get_data( "chstatus", "wire" )
        ...
        for p,bson in results_v.items():
            ...
            evout_chstatus.Insert( ev_chstatus.Status(p) )

        # prepare transfer of truth images, so can have same tick-forward alignment
        for truthimg in ['instance','segment','ancestor','larflow']:
            evout = outlcv.get_data( "image2d", truthimg )
            evin  = io.get_data( "image2d", truthimg )
            for p in range( evin.Image2DArray().size() ):
                evout.Append( evin.Image2DArray().at(p) )
