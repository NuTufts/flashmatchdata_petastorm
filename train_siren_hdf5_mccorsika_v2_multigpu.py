#!/usr/bin/env python3
"""
Multi-GPU Training script for SIREN-based light model with HDF5 FlashMatch data

This script trains a SIREN network to predict PMT light patterns from 3D voxel data
using the new FlashMatch HDF5 dataset format with DistributedDataParallel support.

Usage:
    Single GPU: python train_siren_hdf5_mccorsika_v2_multigpu.py
    Multi-GPU:  torchrun --nproc_per_node=4 train_siren_hdf5_mccorsika_v2_multigpu.py
    or:         python -m torch.distributed.launch --nproc_per_node=4 train_siren_hdf5_mccorsika_v2_multigpu.py
"""
...
print("TRAIN LIGHT-MODEL SIREN WITH HDF5 DATA (MULTI-GPU)")
tstart = time.time()
...
def parse_arguments():
    ...
    parser = ArgumentParser(description='Train SIREN model for flash matching with multi-GPU support')

    parser.add_argument('--config', '-c',
                        type=str,
                        default='config_siren_hdf5_mccorsika_multigpu.yaml',
                        help='Path to YAML configuration file')

    parser.add_argument('--wandb-project',
                        type=str,
                        default='flashmatch-siren-hdf5-multigpu',
...
def load_config(config_path: str) -> Dict[str, Any]:
    ...
    for section, defaults_dict in defaults.items():
        if section in config:
            ...
            config[section] = defaults_dict

    return config


def create_data_loaders(config: Dict[str, Any], is_distributed: bool, rank: int, world_size: int) -> tuple:
    """Create training and validation data loaders with distributed sampler support"""
    ...
    if is_distributed:
        if config['distributed'].get('scale_batch_size', False):
...
def create_models(config: Dict[str, Any], device: torch.device, is_distributed: bool, local_rank: int) -> tuple:
    ...
    if is_distributed:
        ...
        mlp = nn.SyncBatchNorm.convert_sync_batchnorm(mlp)
...
def save_checkpoint(models: Dict[str, nn.Module],
    ...
    # Save model states (unwrap DDP if necessary)
...
def load_checkpoint(checkpoint_path: str,
    ...
    # Load model states (handle DDP wrapping)
    for name, model in models.items():
...
def main():
    ...
    # Override distributed config if running distributed
    if is_distributed:
    ...
    # Create models
    model_config = config['model']
    mlp, siren = create_models(config, device, is_distributed, local_rank)
    ...
    for name, param in siren_for_params.named_parameters():
        if name == "light_yield":
    ...
    init_lr_ly = lr_config['warmup_lr']*lr_config['relative_ly_lr']
    if train_config['freeze_ly_param']:
        init_lr_ly = 0.0
    param_group_list = [
        {"params": param_group_ly,   "lr": init_lr_ly,            "weight_decay":weight_decay*lr_config['relative_ly_lr']},
    ...
    # Setup W&B logging (only on rank 0)
    ...
    for iteration in range(start_iteration, end_iteration):
        ...
        if rank == 0:
            print("siren model returns: ", pe_per_voxel.shape)
        ...
        if train_config['freeze_ly_param']:
            ...
            optimizer.param_groups[0]["lr"] = next_lr*lr_config['relative_ly_lr']  # LY learning rate
